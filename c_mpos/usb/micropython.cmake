# MicroPython USER_C_MODULE for USB host support on ESP32: display adapters
# (Pico_USB_Disp, MIT) plus a minimal USB HID host transport (src/usb_hid.c:
# boot-protocol mice + keyboards on raw usb_host, own client + task, no
# upstream/ changes). ESP32-only: DisplayLink DL-1xx via the in-tree IDF
# usb_host stack.
#
# Pass as:
#   USER_C_MODULE=<repo>/c_mpos/usb/micropython.cmake
...
add_library(usermod_c_usb INTERFACE)

set(C_USB_UPSTREAM ${CMAKE_CURRENT_LIST_DIR}/upstream)

set(C_USB_SOURCES
    ${C_USB_UPSTREAM}/usb_disp.cpp
    ${C_USB_UPSTREAM}/usb_disp_prot_dl-1xx.cpp
    ${C_USB_UPSTREAM}/usb_disp_hal_esp32.cpp
    ${CMAKE_CURRENT_LIST_DIR}/src/usb_mpy.c
    ${CMAKE_CURRENT_LIST_DIR}/src/usb_display_mpy.c
...
if(IDF_TARGET STREQUAL "esp32p4")
    list(APPEND C_USB_SOURCES
        ${C_USB_UPSTREAM}/usb_disp_prot_t6.cpp
        ${C_USB_UPSTREAM}/usb_disp_prot_ms91xx.cpp
    )
endif()

target_sources(usermod_c_usb INTERFACE ${C_USB_SOURCES})
...
set_source_files_properties(${C_USB_SOURCES} PROPERTIES COMPILE_OPTIONS -Os)
...
target_compile_definitions(usermod_c_usb INTERFACE ESP_PLATFORM)

target_include_directories(usermod_c_usb INTERFACE
    ${C_USB_UPSTREAM}
...
foreach(_comp usb esp_timer)
    ...
    endforeach()
    target_include_directories(usermod_c_usb INTERFACE ${_comp_abs_includes})
endforeach()

target_link_libraries(usermod INTERFACE usermod_c_usb)
