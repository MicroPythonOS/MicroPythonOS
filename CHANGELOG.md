...
Board Support:
- ESP32-S3 USB renamed to a stable `usb` API (no backwards compatibility): C module `usb_disp` → `usb` (`USBDisp` → `usb.Display`, `c_mpos/usb_display/` → `c_mpos/usb/`), build flag `--usbdisplay` → `--usb`, Python side behind a new `USBManager` facade (`from mpos import USBManager`; `drivers/display/usb_display/`, `drivers/indev/usb_hid.py` unchanged in place), plus a `frameworks/usb-manager.md` user doc — field units need `--erase-all` or a lib re-sync, stale flash shadows will not fall back to the renamed frozen module
- ESP32-S3: experimental USB display adapter support (tested on DisplayLink DL-1xx, e.g. DL-165/DL-195 — T6/MS91xx protocol code from upstream Pico_USB_Disp is vendored for ESP32-P4 but untested on hardware) via `./scripts/build_mpos.sh esp32s3 --usbdisplay`; drives an external monitor over USB OTG (EDID auto or fixed mode, 640x480 minimum — smaller modes use a sub-25MHz pixel clock real monitors cannot sync to), falling back to the onboard LCD when no adapter is present (MaTouch ESP32-S3 2.8" and Waveshare ESP32-S3-Touch-LCD-2 board files); live-switch between panel and USB at runtime with auto-revert on unplug, plus an IDF usb_host settle delay so hotplugged adapters enumerate instead of wedging
...
- ESP32-S3 USB HID: S3 channel policy for the 8-channel host budget (1 hub + 2 devices max; P4/S31 fit 1 hub + 4 with 16 channels) — display > mouse > keyboard claim order, silent parking with backoff and topology re-arm plus `hid_parked()`/`hid_retry()` introspection, and watchdog idle-reset suppression while HID is parked (claimed devices are skipped port-exactly instead: the sweep resolves open HID handles to (hub, port) and logs the skip inline)
