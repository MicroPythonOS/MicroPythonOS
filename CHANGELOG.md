...
- ESP32-S3 USB display: idle-port auto-reset re-enabled in flap-armed form via `usb_disp.auto_reset_idle()` (default on) — fires only on ports observed to flap since boot, so healthy hub uplinks can never be selected while replugged adapters heal without hands
