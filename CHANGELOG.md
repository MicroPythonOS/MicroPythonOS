...
- ESP32-S3 USB display: idle-port auto-reset retired back to pointer-only after field evidence (an unchirped hub reads full-speed like a stuck adapter, so automatic resets deafened hubs and risked the enumerator abort); episodes now open on disabled ports only, dues defer during bus growth and for 10s after a hub appears, dead hubs decay to a quiet 120s probe, and manual `reset_port()` gains a `force=True` high-speed override
