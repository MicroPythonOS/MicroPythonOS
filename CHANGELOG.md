...
- ESP32-S3 USB display: idle hub ports (enabled but unaddressed, no episode) get one automatic PORT_RESET after a 15s grace via `usb_disp.auto_reset_idle()` (default on, runtime-togglable) instead of staying manual-only; one-shot per plug so healthy devices are never disturbed
