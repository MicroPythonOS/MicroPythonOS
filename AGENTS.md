...
- A Python decorator (`@unittest.skipIf`, `@unittest.skipUnless`, etc.) must be immediately followed by the function or class it decorates. No module-level `if`/`print`/`import` can sit between the decorator and its target. Place skip-explanation prints before the decorator.
...
- Test runner crashes with `OSError: [Errno 5] Input/output error` at `os.read(self.fd, n)` mean the **MicroPython binary died** (PTY slave closed). The I/O error is a symptom — the real question is why the binary crashed. Run the binary directly with `-c` to reproduce and check the exit code: `139` = SIGSEGV, `134` = SIGABRT. E.g.: `cp test.py internal_filesystem/_runner_test.py && lvgl_micropy_unix -X heapsize=32M -c 'import sys; sys.path.insert(0,"lib"); ... ; import unittest; unittest.main(module=_test_mod)'` — look for `Segmentation fault` in the output. The `test_runner.py` retry logic ("Test crashed — retrying...") fires after this crash, but the `OSError` now propagates as a clean `PROCESS CRASHED` message instead of an uncaught traceback.
- Passing a **non-LVGL Python object** (mock, plain class instance) as a parent to any LVGL widget constructor (e.g. `lv.button(mock_obj)`) causes an immediate **segfault** — no Python exception, no traceback, the process just dies. This is the most common cause of test segfaults when tests mock LVGL widgets. If a test must call code that creates LVGL widgets, either mock the calling method entirely (e.g. `detail.add_action_buttons = lambda *a: None`) or use real `lv.obj()` parents (requires LVGL init — graphical test).
...
- Try to make the UI look nice and uniform. For example, buttons in the same flex row should have the same height. 
