...
from com_micropythonos_nostr.nostr_initializer import (
    DM_FETCH_SINCE_MINUTES,
    _dm_subscription_since,
)
...
class TestDmSubscriptionSince(unittest.TestCase):
    ...
    def test_fresh_install_uses_lookback_window(self):
        now = 1700000000
        chats = []
        since = _dm_subscription_since(now, chats, DM_FETCH_SINCE_MINUTES)
        self.assertEqual(since, now - DM_FETCH_SINCE_MINUTES * 60)
