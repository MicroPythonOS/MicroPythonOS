...
class TestFontManagerClearCache(GraphicalTestCase):
    """Tests for FontManager.clear_cache()."""
    ...
    def test_clear_cache_empties_ttf_cache(self):
        """clear_cache() releases every cached TTF font."""
    ...
    def test_clear_cache_allows_reload(self):
        """A TTF font requested after clear_cache() is usable again."""
        FontManager.getFont(size=24, ttf=_TEST_TTF_PATH, emoji=False)
        FontManager.clear_cache()
    ...
    def test_clear_cache_on_empty_caches(self):
        """clear_cache() is safe when nothing was ever cached."""
        FontManager.clear_cache()
        self.assertEqual(len(FontManager._ttf_font_cache), 0)

    def test_clear_cache_keeps_ttf_that_is_on_screen(self):
        """A TTF a live label draws with survives, so the label still renders."""
        font = FontManager.getFont(size=24, ttf=_TEST_TTF_PATH, emoji=False)
        label = lv.label(self.screen)
        label.set_style_text_font(font, lv.PART.MAIN)
        label.set_text("Hello TTF")
        self.wait_for_render()

        FontManager.clear_cache()

        self.assertIs(FontManager.getFont(size=24, ttf=_TEST_TTF_PATH, emoji=False), font)
        label.set_text("Redrawn after clear_cache")
        label.invalidate()
        self.wait_for_render()
        self.assertEqual(label.get_text(), "Redrawn after clear_cache")

    def test_clear_cache_keeps_composed_ttf_that_is_on_screen(self):
        """An emoji font composed over a TTF survives while it is drawn."""
        composed = FontManager.getFont(size=24, ttf=_TEST_TTF_PATH, emoji=True)
        label = lv.label(self.screen)
        label.set_style_text_font(composed, lv.PART.MAIN)
        label.set_text("emoji ❤")
        self.wait_for_render()

        FontManager.clear_cache()

        self.assertIs(FontManager.getFont(size=24, ttf=_TEST_TTF_PATH, emoji=True), composed)
        # The base TTF must be kept too, or the composed font's fallback dangles.
        self.assertTrue(len(FontManager._ttf_font_cache) > 0)
        label.invalidate()
        self.wait_for_render()

    def test_clear_cache_frees_ttf_after_its_label_is_deleted(self):
        """Once the label is gone the TTF is no longer in use and is freed."""
        font = FontManager.getFont(size=24, ttf=_TEST_TTF_PATH, emoji=False)
        label = lv.label(self.screen)
        label.set_style_text_font(font, lv.PART.MAIN)
        label.set_text("Hello TTF")
