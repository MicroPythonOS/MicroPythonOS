    ...
    if current_key is None:
        ...
        if elapsed >= REPEAT_INITIAL_DELAY_MS and since_last_repeat >= REPEAT_RATE_MS:
            ...
            if current_key == lv.KEY.PREV:
                if __debug__: logger.debug("Repeated PREV does not do anything, instead it triggers ESC (back) if long enough")
                if since_last_repeat > REPEAT_PREV_BECOMES_BACK:
                    if __debug__: logger.debug("back button trigger!")
                    data.key = lv.KEY.ESC
                    ...
                    if __debug__: logger.debug("repeat PREV ignored because not pressed long enough")
            else:
                if __debug__: logger.debug("Send a new PRESSED/RELEASED pair for repeat")
                data.key = current_key
                data.state = next_state
                last_key = current_key
                last_state = data.state
