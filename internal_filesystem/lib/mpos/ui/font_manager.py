...
"""
    ...
    @classmethod
    def clear_cache(cls):
        """Release the C-heap fonts an app allocated: its TTFs and the emoji
        imgfonts composed on top of them. Both come from lv_malloc(), which the
        MicroPython GC does not manage, so dropping the Python reference alone
        leaks them and fragments the C heap.

        A font that a reachable widget still draws with is kept, so calling this
        at any time costs at most a reload later. It never frees memory that
        LVGL still reads.
        ...
        if not cls._ttf_font_cache:
            # Nothing to free, so skip the widget walk. Most apps never load a
            # TTF, which makes this the common case on every teardown.
            return

        fonts_in_use = cls._font_pointers_in_use()

        # A TTF is removable only when neither it nor an emoji font composed on
        # top of it is still drawn.
        removable_ttf_ids = set()
        for font in cls._ttf_font_cache.values():
            pointer = cls._font_pointer(font)
            if pointer is not None and pointer not in fonts_in_use:
                removable_ttf_ids.add(cls._font_identity(font))
        for cache_key in cls._composed_font_cache:
            composed_font = cls._composed_font_cache[cache_key]
            if cls._font_pointer(composed_font) in fonts_in_use:
                removable_ttf_ids.discard(cache_key[0])

        for cache_key in list(cls._composed_font_cache.keys()):
            if cache_key[0] not in removable_ttf_ids:
                continue
        ...
        for cache_key in list(cls._ttf_font_cache.keys()):
            font = cls._ttf_font_cache[cache_key]
            if cls._font_identity(font) not in removable_ttf_ids:
                continue
            del cls._ttf_font_cache[cache_key]
            try:
    ...
    @classmethod
    def _font_pointer(cls, font):
        """The raw lv_font_t* as bytes, or None when it cannot be read.

        LVGL hands back a fresh wrapper object every time a style getter returns
        a font, so id() cannot match one against a cached font. The wrapper's
        bytes are the C pointer itself, which does match.
        """
        try:
            return bytes(font)
        except Exception:
    ...
    @classmethod
    def _font_pointers_in_use(cls):
        """Pointers of the fonts that reachable widgets currently draw with."""
        pointers = set()
        pending = cls._widget_roots()
        while pending:
            obj = pending.pop()
            for part in (lv.PART.MAIN, lv.PART.ITEMS):
                try:
                    pointer = cls._font_pointer(obj.get_style_text_font(part))
                except Exception:
                    continue
    ...
    @classmethod
    def _widget_roots(cls):
        """Every widget tree that can still be drawn: the layers, the screens a
        load animation is running between, and the activity screens on the stack.
        """
        roots = []
        for get_root in (lv.screen_active, lv.layer_top, lv.layer_bottom, lv.layer_sys):
            try:
        ...
        try:
            ...
            roots.append(display.get_screen_loading())
            roots.append(display.get_screen_prev())
        except Exception:
            pass
        try:
            from .view import screen_stack
            for entry in screen_stack:
